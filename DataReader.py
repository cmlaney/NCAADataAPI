    ...
    if hasattr(game, 'playByPlay'):
        for period, possessions in game.playByPlay.periods.items():
            ...
            for possession in possessions:
                home = 0
                if possession.teamName == game.homeTeam.nameRaw:
                    home=1
                c.execute('insert into possession (game_id, team_name, time, period, home) values ({}, "{}", "{}", {}, {})'\
                          .format(last_game_id, possession.teamName, possession.time, period_int, home))
...
def fetchGames(week, refetch=False, store=False):
    ...
    for group in output['scoreboard']:
        ...
        for game in group['games']:
            ...
            if len(key) > 0:
                games[key] = newGame
                if store and not checkIfGameExists(newGame.url):
